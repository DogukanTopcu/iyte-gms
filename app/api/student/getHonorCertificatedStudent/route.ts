...
export async function GET(req: NextRequest) {
  ...
  const filterHonorStudents = (students: StudentWithGpaAndTerm[]) => {
    // Filter students with GPA between 3.0 and 3.5 (exclusive) for Honor Certificate
    return students.filter(student => student.gpa >= 3.0 && student.gpa < 3.5);
  };
  ...
  if (role === 'department secretariat') {
    ...
    const honorStudents = filterHonorStudents(studentsData);
    const sortedStudents = sortStudents(honorStudents);
